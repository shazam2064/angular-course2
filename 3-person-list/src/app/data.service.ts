...
export class DataService {
  ...
  cargarPersonas() {
    return this.httpClient.get('https://person-list-f58ec.firebaseio.com/datos.json');
  }

  // Guarda todo el arreglo de personas
  guardarPersonas(personas: Persona[]) {
    this.httpClient.put('https://person-list-f58ec.firebaseio.com/datos.json', personas)
      .subscribe(
        (response) => {
  ...
  modificarPersona(index: number, persona: Persona) {
    let url: string;
    url = 'https://person-list-f58ec.firebaseio.com/' + '/datos/' + index + '.json';
